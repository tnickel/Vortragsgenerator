...
class VortragGeneratorApplicationTests {
    ...
    @Transactional
    @org.springframework.test.annotation.Commit
    void generateCybersecurityV2Example() throws Exception {
        System.out.println("=== STARTING CYBERSECURITY V2 EXAMPLE GENERATION ===");
        ...
        // 1. Read JSON file
        String jsonPath = "src/main/resources/examples/cybersecurity_v2_slides.json";
        File jsonFile = new File(jsonPath);
        if (!jsonFile.exists()) {
            throw new IllegalStateException("Cybersecurity v2 slides JSON file not found at: " + jsonFile.getAbsolutePath());
        }

        JsonNode root = objectMapper.readTree(jsonFile);
        String name = root.path("title").asText("cybersecurity Teil 1 Anfänger - Didaktische Version");
        String subtitle = root.path("subtitle").asText("");
        ...
        Project project = new Project();
        project.setName(name);
        project.setPrompt("Didaktisch optimierte Version von Cybersecurity für Anfänger. Mit Storytelling (Anna), Reflexionsfragen, konkreten Beispielen und langsamerem Sprechtempo.");
        project.setStatus("GENERATING_PPTX");
        project.setModelName("Antigravity-Offline");
        project = projectRepository.save(project);
        ...
        try {
            ...
            project.setStatus("COMPLETED");
            projectRepository.save(project);
            System.out.println("=== CYBERSECURITY V2 GENERATION COMPLETED SUCCESSFULLY ===");
        } catch (Exception e) {
            System.err.println("Failed in Audio/Video generation stage: " + e.getMessage());
            e.printStackTrace();
            project.setStatus("FAILED");
